...
searchButtonEl.addEventListener("click", function () {
  ...
  wizardBio["name"] = wizardNameInputEl.value
    .trim()
    .substring(0, 7)
    .toUpperCase();
  ...
  // if 2 or more than 2 characters are entered
    ...
    if (
      wizardBio["name"].length === 2 &&
      wizardBio["name"][0] === "." &&
      wizardBio["name"][1] === "."
    ) {
      // prompt the wizard to enter at least 2 letters
      errorMsgParaEl.innerText =
        "Cannot enter 2 consecutive periods/dots/fullstops. " +
...
clickMeButtonEl.addEventListener("click", function () {
  ...
  // if the mentor and the wizard belong to different houses
    ...
    if (mentorBio.house === "") {
      ...
      if (
        (mentorBio.ancestry === "muggle" || mentorBio.species === "human") &&
...
acceptContractButtonEl.addEventListener("click", function () {
  ...
  contractText +=
    "<hr>Signed & Sealed: " + wizardBio.mentor + " & " + wizardBio.name;
